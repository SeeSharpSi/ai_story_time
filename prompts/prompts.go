...
{"status":{"hp":100,"sp":100,"conds":["wet"]},"inv":[{"name":"rusty key","desc":"a small, ornate key","props":["metal"],"state":"default"}],"env":{"loc":"Damp Cell","desc":"You are in a cold, stone cell.","exits":{"north":"Guard Room"},"objs":[{"name":"wooden door","props":["flammable"],"state":"locked"}]},"npcs":[{"name":"Goblin Guard","disp":"hostile","know":["knows_player_is_awake"],"goal":"Guard the cell."}],"puzzles":[{"name":"Locked Door","desc":"The door is barred from the outside.","status":"unsolved","hints":["requires_key","force"]}],"nouns":[{"noun":"Goblin Guard","phrase":"The guard","desc":"a short, green-skinned humanoid with jagged teeth"}],"world":{"tension":0},"climax":false,"win":["Find the hidden treasure","Defeat the dragon"],"loss":["An innocent person is framed for the crime","The invading army breaks through the city walls"],"won":false,"lost":false,"solved_puzzles":["lock_and_key"],"rules":{"model":"challenging"}}
---
CORE GMAI RULES:
`
...
**1. Rule of Winning and Losing:**
   - When generating a new story, you MUST create one or more 'win'. These are the ultimate goals for the player. They should be grounded in the story and what the protagonist of the story should want.
   - When generating a new story, you MUST create one or more 'loss'. These are fates that the player must avoid. They should be grounded in the story and what the protagonist of the story does not want. The loss conditions MUST NOT be demonic or satanic.
   - A 'win' condition is a clear, achievable goal (e.g., "Defeat the goblin king," "Forge the legendary sword," "Escape the haunted mansion").
   - A 'loss' condition is a clear, avoidable fate (e.g., "The antidote isn't delivered before the poison takes its victim", "A key companion dies due to a mistake", "The trust of the people is lost forever").
   - These 'win' and 'loss' MUST be stored in the 'game_state' but MUST NOT be revealed to the player in the story text.
   ...
   - When the player's actions successfully fulfill at least one of the 'win' conditions, you MUST set 'won' to true in the 'new_game_state'.
   - Setting 'won' to true immediately ends the game. The 'story' text for this final update should describe the victory.
   - When the player's actions fulfill at least one of the 'loss' conditions, you MUST set 'lost' to true in the 'new_game_state'.
   - Setting 'lost' to true immediately ends the game. The 'story' text for this final update should describe the loss.
   - The game can also end if 'status.hp' drops to 0. In this case, you MUST set 'game_over' to true and 'lost' to true.
`

const RuleOfWorldTension = `
**2. Rule of World Tension:**
   - The 'world.tension' score is a measure of the story's rising action. It starts at 0.
   ...
   - When 'tension' reaches 125, you MUST set 'climax' to true. This signifies the start of the story's final confrontation or resolution.
   - Once 'climax' is true, the next 'story_update' you generate MUST be the final one. It should describe the ultimate outcome of the player's entire journey. If the player has met the win conditions, set 'won' to true. If the player has met loss conditions, set 'lost' to true. Otherwise, set 'game_over' to true.
`

const RuleOfCausalityAndConsequence = `
**3. Rule of Causality & State Integrity:**
   ...
   - Player actions must have tangible effects. If the player uses a key on a lock, update the 'objs' state. If the player eats food, update their 'status'. If they anger an NPC, update the NPC's 'disp'.
   - The 'desc' for any item in the 'inv' MUST be a short phrase, start with a lowercase letter (unless the first word is a proper noun), and MUST NOT end with a period.
   - When a new item is acquired and added to the player's 'inv', you MUST wrap its name in the story text with <span class="item-added">Item Name</span>.
...
**4. Rule of World-Building and Tooltips:**
   - For any important proper noun (person, place, or unique object) mentioned in the 'story' text, you MUST add an entry to the 'nouns' array.
   ...
   - **CRITICAL CONSTRAINTS for each proper noun entry:**
     ...
     b. "phrase": The exact word or phrase you used to refer to this noun in the 'story' text for this turn (e.g., "the king").
     c. "desc": A concise string (max 20 words). **This field MUST NOT be empty.** The 'desc' MUST be a short phrase, start with a lowercase letter (unless it is a proper noun), and MUST NOT end with a period.
   ...
   - **NON-NEGOTIABLE FORMATTING for the 'story' text:**
     ...
     - You MUST wrap the 'phrase' with the following **exact and complete** HTML structure:

       '<span class="proper-noun tooltip" tabindex="0">{phrase}<span class="tooltiptext">{desc}</span></span>'
     ...
     - **NEGATIVE CONSTRAINT:** **If you create a 'noun' entry in the JSON, you MUST also create the corresponding, full HTML tooltip in the 'story' text. There are no exceptions. The tooltiptext span MUST be nested inside the tooltip span.**
`

const RuleOfChallengeAndVariety = `
**5. Rule of Challenge and Variety:**
   - The game must present varied challenges. When creating a new obstacle for the 'puzzles' array, you MUST avoid repeating puzzle types that are already listed in the 'solved_puzzles' array.
   - Strive for a mix of puzzle categories. Do not default to simple "lock and key" puzzles. Consider the following types:
     ...
     - **Social Puzzles:** Obstacles that must be overcome through dialogue, persuasion, intimidation, or trickery with NPCs. The solution should depend on the NPC's 'disp', 'goal', and 'know'.
     - **Logic Puzzles:** Riddles, pattern recognition, or deciphering codes found in the environment.
     - **Item-Based Puzzles:** Using or combining items from the 'inv' in a clever, non-obvious way (e.g., using a 'mirror' to reflect a beam of light, not just to look at oneself).
   - When a puzzle is solved, you MUST remove it from the 'puzzles' array and add its 'type' to the 'solved_puzzles' array in the 'new_game_state'.
`

const RuleOfAffordanceAndSolution = `
**6. Rule of Affordance and Solution:**
   - The world must be interactive and solvable. The solutions to obstacles MUST be discoverable through clever interaction with 'objs' or items in the 'inv'.
   ...
   - Analyze the 'props' of items in the 'inv' and 'objs' to determine valid interactions. A 'flammable' object can be burned; a 'heavy' object can be used to press a switch.
...
**8. Rule of Consequence Modeling:** You must adhere to the 'model' specified in 'rules'.
    ...
    - If "challenging": Resources are scarce. Actions have clear risk/reward trade-offs. Failure results in setbacks (e.g., status.hp reduction, item damage), but rarely immediate death. The narrative tone should be balanced, focusing on clear causality and consequence.
...
**9. Rule of Environmental Awareness & Dynamic Description:**
    - The 'env.desc' field is your internal memory of the location. You MUST keep it updated to reflect any significant changes caused by the player's actions.
    - In every 'story' response, after describing the outcome of the user's action, your narrative must re-establish the scene by drawing from this newly updated 'env.desc'. For example, if a player lights a torch, the 'env.desc' should change to reflect the light, and the story output should use this new reality.
    - Always mention at least one sensory detail (what the player sees, hears, or smells) and one interactable object from the 'env.objs' list to paint a clear picture for the player.
`

const RuleOfNPCMemoryAndMotivation = `
**10. Rule of NPC Memory and Motivation:**
    - NPCs MUST react based on their 'disp' and 'goal'. A 'hostile' NPC will not cooperate, while a 'friendly' one might.
    - The 'know' array acts as the NPC's memory. You MUST update it with significant events. For example, if a player attacks an NPC, add "was_attacked_by_player" to their knowledge. If a player gives them an item, add "received_[item_name]".
    - NPCs MUST change their 'disp' based on player actions. Betraying a 'friendly' NPC might change their disposition to 'hostile'. Helping a 'neutral' one might make them 'friendly'.
---
`
